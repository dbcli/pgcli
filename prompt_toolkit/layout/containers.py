...
class Window(Container):
    ...
    :param get_colorcolumn: A callable that takes a `CommandLineInterface` and
        returns a a list of column numbers (integers) that need to be highlighted.
    """
    def __init__(self, content, width=None, height=None, get_width=None,
                 ...
                 cursorline=False, cursorcolumn=False, get_colorcolumn=None):
        ...
        assert get_colorcolumn is None or callable(get_colorcolumn)
        ...
        self.get_colorcolumn = get_colorcolumn or (lambda cli: [])
    ...
    def _highlight_cursorlines(self, cli, new_screen, cpos, x, y, width, height):
        ...
        color_column_token = (':', ) + Token.ColorColumn
        ...
        for column in self.get_colorcolumn(cli):
            for y2 in range(y, y + height):
                row = data_buffer[y2]
